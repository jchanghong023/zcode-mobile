...
| 下载              | HTTP/HTTPS 地址由 `setDownloadListener` → 系统 `DownloadManager`（公共 Downloads 目录）；WebView 生成的文件由 Android 保存文件对话框写入用户选择的位置                                                                      | API≤28 的公共目录下载需在用户发起时申请 WRITE_EXTERNAL_STORAGE；`blob:` 不交给只接受 HTTP/HTTPS 的 DownloadManager。用户取消保存对话框不写入文件                                                                                                                                                                  |
