...
- 项目列表的来源由远控入口统一合成：bootstrap 工作区先到，`recentProjects` 可异步补充；列表注入只确保侧栏可见，不改变当前工作区。连接换代后，旧连接返回的设置结果不得进入新列表。
